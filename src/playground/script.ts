...
import type { Beat, ReplayScript } from './replay.ts'

/**
 * What replay plays — the corpus, with no machinery in it.
 *
 * Split from the transport because the two touched at exactly two lines and
 * nothing else: the script never calls the transport, and the transport reads
 * the script only as data. Every ticket that earns a case adds it here, and no
 * ticket adding a case has to open the file that owns `createEventSource`.
 *
 * These are values. A test that only wants to know what `reduce` makes of a
 * beat imports this and never loads a transport at all.
 */
 ...
 */
export const HELD = new Map<string, string>([
...
/** The whole corpus, as the transport takes it. */
export const SCRIPT: ReplayScript = { opening: OPENING, answer: reply, held: HELD }
